...
from . import BOTLOG, BOTLOG_CHATID, ultronub
...
if Config.PLUGIN_CHANNEL:
    ...
    async def install():
        documentss = await ultronub.get_messages(
        ...
        for module in range(total):
            ...
            if os.path.exists(f"userbot/plugins/{plugin_name}"):
                return
            downloaded_file_name = await ultronub.download_media(
                await ultronub.get_messages(Config.PLUGIN_CHANNEL, ids=plugin_to_install),
                "userbot/plugins/",
            )
    ...
    ultronub.loop.create_task(install())
